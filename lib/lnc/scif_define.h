...
#define SCIF_MEM_SIZE_TT   45   //TT點個數
#define SCIF_MEM_SIZE_CT   46   //CT點個數
#define SCIF_MEM_SIZE_R    47   //R點個數
#define SCIF_MEM_SIZE_TV   48   //TV點個數
#define SCIF_MEM_SIZE_TS   49   //TS點個數
#define SCIF_MEM_SIZE_CV   50   //CV點個數
#define SCIF_MEM_SIZE_CS   51   //CS點個數
...
//命令種類
#define SC_DEFAULT_CMD      0  // Default Command -- 當要在畫面上同時顯示多個控制器的資訊時，應使用此種封包
...
typedef union tag_SC_DATA
{
    ...
    SC_POINTER     Ptrs[MAX_PTR_NUM];      //指標 的 值
}SC_DATA;
...
typedef struct tag_FUNCTION_SETTING
{
    ...
    unsigned int  MediaIPLong1;
    unsigned int  MediaIPLong2;
    unsigned char HaveMediaFunction;       //有使用媒合主機功能
    unsigned char Reserve1;       //保留1
    unsigned char Reserve2;       //保留2
    unsigned char Reserve3;       //保留3
}FUNCTION_SETTING;
...
{
    unsigned int   MakerID;            //製造商編號
    unsigned char  SecurBits[768];     //功能啟用 Bit
}FUNC_SETTING;



//-----程式呼叫清單
typedef struct tag_PROCESS_ITEM
{
    char  Name[32];     //程序0
    int   Size;
}PROCESS_ITEM;
