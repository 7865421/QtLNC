...
void ExcelBase::saveAs(const QString& filename)
{
    ...
    if ( d->book != NULL && ! d->book->isNull() )
    {
        ...
        qDebug()<<strPath;
        d->book->dynamicCall("SaveAs(const QString&)", QDir::toNativeSeparators(filename));
    }
#else
    Q_UNUSED(filename)
#endif // Q_OS_WIN
...
void ExcelBase::setVisible(bool value)
{
    ...
    if (d->excel != NULL && !d->excel->isNull())
    {
      ...
      //  d->excel->dynamicCall("SetValue(const QVariant&)",value);
    }
#else
    Q_UNUSED(value)
#endif // Q_OS_WIN
...
bool ExcelBase::setCurrentSheetName(int index,QString name)
{
    bool ret = false;
#if defined(Q_OS_WIN)
    Q_D(ExcelBase);
...
bool ExcelBase::Autofit()
{
    ...
    Q_D(ExcelBase);
        ...
        QAxObject * cells = urange->querySubObject("Columns");
        cells->dynamicCall("AutoFit");
        urange->setProperty("HorizontalAlignment",-4108);
        urange->setProperty("VerticalAlignment",-4108);
       // delete urange;
       // delete cells;
        ret = true;
#endif // Q_OS_WIN
